...
import (
	...
	"github.com/go-toho/contrib/config/mapstructureo"
)

func testMustDo(t testing.TB, what string, err error) {
	t.Helper()
	if err != nil {
...
func TestStrings(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			Expected: StringsStruct{
				S: mapstructureo.Strings{"the", "quick", "brown"},
			},
		}, {
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			dc := mapstructure.DecoderConfig{
				DecodeHook: mapstructureo.DecodeStrings,
				Result:     &s,
			}
			decoder, err := mapstructure.NewDecoder(&dc)
			testMustDo(t, "new decoder", err)
			err = decoder.Decode(c.Source)
			testMustDo(t, "decode", err)
...
func TestDecodeStringToMap(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			ExpectedErr: mapstructureo.ErrInvalidKeyValuePair,
		},
	}
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			dc := mapstructure.DecoderConfig{
				DecodeHook: mapstructureo.DecodeStringToMap(),
				Result:     &result,
			}
			decoder, err := mapstructure.NewDecoder(&dc)
			testMustDo(t, "new decoder", err)
...
func TestOnlyString(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			Err:    mapstructureo.ErrMustBeString,
		}, {
			Name:   "Struct",
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			dc := mapstructure.DecoderConfig{
				DecodeHook: mapstructureo.DecodeOnlyString,
				Result:     &o,
			}
			decoder, err := mapstructure.NewDecoder(&dc)
			testMustDo(t, "new decoder", err)
...
func TestStringToSliceWithBracketHookFunc(t *testing.T) {
	pointerString := func(s string) *string {
		if s == "" {
			return nil
		}
		return &s
	...
	cases := []struct {
		...
		},
		{
			...
			Source:     `{"key": "value"}`,
			Expected:   []string(nil),
			ErrMessage: pointerString("source data must be an array or slice"),
		},
		{
			Name:       "invalid json",
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			...
			dc := mapstructure.DecoderConfig{
				DecodeHook: mapstructureo.StringToSliceWithBracketHookFunc(),
				Result:     &result,
			}
			decoder, err := mapstructure.NewDecoder(&dc)
			testMustDo(t, "new decoder", err)
...
func TestStringToStructHookFunc(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.Name, func(t *testing.T) {
			dc := mapstructure.DecoderConfig{
				DecodeHook: mapstructureo.StringToStructHookFunc(),
				Result:     &c.Result,
			}
			decoder, err := mapstructure.NewDecoder(&dc)
			testMustDo(t, "new decoder", err)
